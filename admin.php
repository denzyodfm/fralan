...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['login'])) {
  $configuredUser = trim($env['WP_ADMIN_USER'] ?? '');
  $configuredPassword = $env['WP_ADMIN_PASSWORD'] ?? '';
  if ($configuredUser === '' || $configuredPassword === '') {
    $error = 'Admin access is disabled until WP_ADMIN_USER and WP_ADMIN_PASSWORD are configured.';
  } elseif (hash_equals($configuredUser, $_POST['username'] ?? '') && hash_equals($configuredPassword, $_POST['password'] ?? '')) {
    session_regenerate_id(true);
    $_SESSION['archive_admin'] = true;
    $_SESSION['csrf'] = bin2hex(random_bytes(24));
    header('Location: admin.php'); exit;
  } else $error = 'The username or password is incorrect.';
}
...
$typeSettingsFile = __DIR__.'/data/site.json';
$typeSettings = file_exists($typeSettingsFile) ? json_decode(file_get_contents($typeSettingsFile), true) : [];
$contentTypes = normalizeContentTypes(is_array($typeSettings) ? $typeSettings : []);
$contentTypeKeys = array_column($contentTypes, 'key');
function slugify($text) { $text = strtolower(trim($text)); $text = preg_replace('/[^a-z0-9]+/', '-', $text); return trim($text, '-') ?: uniqid('entry-'); }
function cleanBody($html) {
  $allowed = ['p','br','h2','h3','strong','em','blockquote','ul','ol','li','a'];
  ...
  foreach (iterator_to_array($document->getElementsByTagName('*')) as $node) {
    ...
    foreach (iterator_to_array($node->attributes ?? []) as $attribute) {
      $safeHref = $node->tagName === 'a' && $attribute->name === 'href' && preg_match('~^(https?://|mailto:|/|\?|#)~i', trim($attribute->value));
      if (!$safeHref) $node->removeAttribute($attribute->name);
    }
    if ($node->tagName === 'a') $node->setAttribute('rel', 'noopener noreferrer');
  }
  $root = $document->documentElement; $clean = '';
  foreach (iterator_to_array($root?->childNodes ?? []) as $child) $clean .= $document->saveHTML($child);
  return trim($clean);
}
function uniqueSlug($title, $items, $currentId) {
  $base = slugify($title); $slug = $base; $suffix = 2;
  $used = array_column(array_filter($items, fn($item) => ($item['id'] ?? '') !== $currentId), 'slug');
...
if ($loggedIn && $_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['login'])) {
  ...
  if (isset($_POST['delete'])) {
    ...
    elseif ($body === '') $error = 'Please add the content body before saving.';
      ...
      $image = $_POST['existing_image'] ?? 'assets/images/archive-study.jpg';
      ...
      if (!$error) {
        $entryType = in_array($_POST['type'] ?? '', $contentTypeKeys, true) ? $_POST['type'] : 'transliteration';
        $entry = ['id'=>$id,'slug'=>uniqueSlug($title, $items, $id),'type'=>$entryType,'title'=>$title,'excerpt'=>$excerpt,'category'=>$category,'language'=>trim($_POST['language'] ?? ''),'date'=>$_POST['date'] ?? date('Y-m-d'),'image'=>$image,'imageAlt'=>trim($_POST['imageAlt'] ?? ''),'audio'=>$audio,'featured'=>isset($_POST['featured']),'body'=>$body];
...
<body><link rel="stylesheet" href="assets/css/admin-modern.css"><script>document.addEventListener('DOMContentLoaded',()=>{const types=<?=json_encode($contentTypes,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES)?>,select=document.querySelector('select[name=type]'),selected=<?=json_encode($edit['type']??'transliteration')?>;if(select){select.replaceChildren(...types.map(type=>{const option=document.createElement('option');option.value=type.key;option.textContent=type.label;option.selected=type.key===selected;return option}))}const nav=document.querySelector('header nav');if(nav){const view=nav.querySelector('a[target]');[['content-types.php','Content types'],['manual.php','Help']].forEach(([href,label])=>{const link=document.createElement('a');link.href=href;link.textContent=label;nav.insertBefore(link,view)})}})</script>
