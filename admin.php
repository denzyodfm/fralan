...
if ($loggedIn && $_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['login'])) {
  ...
  if (isset($_POST['delete'])) {
    ...
    elseif ($body === '') $error = 'Please add the content body before saving.';
      ...
      if (!$error) {
        $entry = ['id'=>$id,'slug'=>uniqueSlug($title, $items, $id),'type'=>($_POST['type'] ?? '') === 'article'?'article':'transliteration','title'=>$title,'excerpt'=>$excerpt,'category'=>$category,'language'=>trim($_POST['language'] ?? ''),'date'=>$_POST['date'] ?? date('Y-m-d'),'image'=>$image,'imageAlt'=>trim($_POST['imageAlt'] ?? ''),'audio'=>$audio,'featured'=>isset($_POST['featured']),'body'=>$body];
...
<body><link rel="stylesheet" href="assets/css/admin-modern.css">
